...
const MyBookingsPage = () => {
  ...
  // ✅ FIX: Provide a fallback empty array `[]` to prevent error on initial render
    ...
    <div className="bg-slate-50 min-h-screen font-sans">
      <div className="max-w-5xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <header className="mb-8">
          <h1 className="text-4xl font-bold text-slate-800 tracking-tight">My Bookings</h1>
          <p className="text-slate-500 mt-2 text-lg">Track and manage all your service requests in one place.</p>
        </header>
        ...
        <main>
          ...
          {loading && (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 mt-4">Fetching your bookings...</p>
            </div>
          )}
          ...
          {/* 7. Display Bookings or Empty State */}
          {!loading && !error && (
            <>
              {filteredBookings.length > 0 ? (
                <ul className="space-y-6">
                  {filteredBookings.map((booking) => (
                    <li key={booking._id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow duration-300">
                      <Link to={`/booking/${booking._id}`} className="block p-5 sm:p-6">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                          <div className="flex items-center gap-4">
                            <img src={booking.provider.profilePic?.url || 'https://placehold.co/100x100/E2E8F0/4A5568?text=??'} alt={booking.provider.name} className="w-14 h-14 rounded-full object-cover border-2 border-white ring-2 ring-slate-100" />
                            <div>
                              <p className="font-semibold text-slate-800 text-lg">{booking.provider.name}</p>
                              {/* The serviceType is on the provider object in your backend response */}
                              <p className="text-slate-500 text-sm">{booking.provider.serviceType}</p>
                            </div>
                          </div>
                ...
                <div className="text-center py-20 px-6 bg-white rounded-lg border-2 border-dashed">
                    ...
                    <Link to="/services" className="mt-6 inline-block bg-blue-600 text-white font-semibold px-5 py-2 rounded-lg hover:bg-blue-700 transition-colors">
                      Find a Service
                    </Link>
                </div>
              )}
